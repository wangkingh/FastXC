...
int main(int argc, char **argv)
{
    ...
    int nseg_1x = cal_nseg(argument.seglen, npts, delta); // Calculate the number of points of sement
    int nstep = npts / nseg_1x;

    int nseg_2x = nseg_1x * 2; // Create nseg_2x for zero padding data
    int nspec_output = nseg_2x / 2 + 1;

    float df_1x = 1.0 / (nseg_1x * delta);
    ...
    FilterResp *myResp = processButterworthFilters(filter, filter_count, df_2x, nseg_2x);    // Calculate filter f domain response
    ...
    size_t d_batch = EstimateGpuBatch(gpu_id, npts, nseg_1x, nstep_valid, num_ch, filter_count, wh_flag, do_runabs_mf || do_runabs);
    h_batch = (h_batch > nValid_batch) ? nValid_batch : h_batch;
    d_batch = (d_batch > h_batch) ? h_batch : d_batch;
    h_batch = d_batch;
    size_t batch = 0.7 * h_batch;
    ...
    AllocateGpuMemory(batch, nseg_1x, num_ch, do_runabs || do_runabs_mf, wh_flag,
    ...
    for (int i = 0; i < filter_count; i++)
    {
        ...
        CUDACHECK(cudaMemcpy(d_responses + i * nseg_2x, myResp[i].response, nseg_2x * sizeof(cuComplex), cudaMemcpyHostToDevice));
    }
    ...
    for (size_t finish_batch = 0; finish_batch < nValid_batch; finish_batch += batch)
    {
        ...
        DimCompute(&dimgrd_1x, &dimblk_1x, nseg_1x, proc_cnt);
        ...
        for (int stepidx = 0, done_step = 0; stepidx < nstep; stepidx++)
        {
            ...
            CUDACHECK(cudaMemset(d_sacdata, 0, proc_cnt * nseg_1x * sizeof(float)));
            CUDACHECK(cudaMemset(d_spectrum, 0, proc_cnt * nseg_1x * sizeof(cuComplex)));
            ...
            size_t fwidth_2x = 0.5 * nseg_2x + 1; // set to 0.5 * nseg_2x + 1, because of the zero-padding
            DimCompute(&c_fdimgrd, &c_fdimblk, fwidth_2x, proc_cnt);
            filterKernel<<<c_fdimgrd, c_fdimblk>>>(d_spectrum_2x, d_responses, nseg_2x, fwidth_2x, proc_cnt);
            CUFFTCHECK(cufftExecC2R(planinv_2x, (cufftComplex *)d_spectrum_2x, (cufftReal *)d_sacdata_2x));
            InvNormalize2DKernel<<<dimgrd_2x, dimblk_2x>>>(d_sacdata_2x, nseg_2x, nseg_2x, proc_cnt, delta);

            // cut the data to nseg length
            CUDACHECK(cudaMemcpy2D(d_sacdata, nseg_1x * sizeof(float), d_sacdata_2x, nseg_2x * sizeof(float), nseg_1x * sizeof(float), proc_cnt, cudaMemcpyDeviceToDevice));

            if (wh_before)
            {
                CUFFTCHECK(cufftExecR2C(planfwd, (cufftReal *)d_sacdata, (cufftComplex *)d_spectrum));
                FwdNormalize2DKernel<<<dimgrd_1x, dimblk_1x>>>(d_spectrum, nseg_1x, nseg_1x, proc_cnt, delta);
                freqWhiten(d_spectrum, d_weight, d_tmp_weight, d_tmp, num_ch, nseg_1x, proc_batch, delta, f_idx1, f_idx2, f_idx3, f_idx4);
                cisnan2DKernel<<<dimgrd_1x, dimblk_1x>>>(d_spectrum, nseg_1x, nseg_1x, proc_cnt);
                CUFFTCHECK(cufftExecC2R(planinv, (cufftComplex *)d_spectrum, (cufftReal *)d_sacdata));
                InvNormalize2DKernel<<<dimgrd_1x, dimblk_1x>>>(d_sacdata, nseg_1x, nseg_1x, proc_cnt, delta);
            }
            ...
            if (do_runabs_mf)
            {
                runabs_mf(d_sacdata, d_filtered_sacdata, d_total_sacdata,
                          ...
                          d_weight, d_tmp_weight,
                          &planinv, freq_lows,
                          filter_count, delta, proc_batch, num_ch, MAXVAL, nseg_1x, nseg_2x, &planinv_2x, &planfwd_2x);
                CUDACHECK(cudaMemcpy2D(d_sacdata, nseg_1x * sizeof(float),
                                       d_total_sacdata, nseg_1x * sizeof(float),
                                       nseg_1x * sizeof(float), proc_cnt, cudaMemcpyDeviceToDevice));
            }

            if (do_onebit)
            {
                onebit2DKernel<<<dimgrd_1x, dimblk_1x>>>(d_sacdata, nseg_1x, nseg_1x, proc_cnt);
            }

            if (do_runabs)
            {
                float freq_lows_limit = freq_low * 0.667;
                runabs(d_sacdata, d_tmp, d_weight, d_tmp_weight, freq_lows_limit, delta, proc_batch, num_ch, nseg_1x, MAXVAL);
            }
