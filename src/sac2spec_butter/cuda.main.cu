...
int main(int argc, char **argv)
{
    ...
    int filter_count = 0;
    ButterworthFilter *filters = readButterworthFilters(argument.filter_file, &filter_count); // read in filter file
    ...
    size_t d_batch = EstimateGpuBatch(gpu_id, nseg, num_ch);
    ...
    float *d_sac_sum = NULL;    // segment sacdata in GPU, for store sum result
    float *d_sacdata_2x = NULL; // 2x  length segment sacdata in GPU

    float *d_filtered = NULL;      // filtered sac data
    float *d_filtered_tmp = NULL;  // tmp filtered sac data,for reverse
    float *d_sac_hist = NULL;      // history of sac data
    float *d_filtered_hist = NULL; // history of filtered data
    double *d_a = NULL;            // a of filter
    double *d_b = NULL;            // b of filter
    ...
    AllocateGpuMemory(batch, nseg, num_ch,
                      &d_sacdata, &d_sac_sum, &d_spectrum,
                      &d_sacdata_2x, &d_spectrum_2x,
                      &d_filtered, &d_filtered_tmp,
                      &d_sac_hist, &d_filtered_hist, &d_a, &d_b,
                      &d_tmp,
    ...
    for (size_t finish_batch = 0; finish_batch < nValid_batch; finish_batch += batch)
    {
        ...
        for (int stepidx = 0, done_step = 0; stepidx < nstep; stepidx++)
        {
            ...
            CUDACHECK(cudaMemcpy(d_a, filters[0].a, 5 * sizeof(double), cudaMemcpyHostToDevice));
            CUDACHECK(cudaMemcpy(d_b, filters[0].b, 5 * sizeof(double), cudaMemcpyHostToDevice));

            CUDACHECK(cudaMemset(d_sac_hist, 0, proc_cnt * 5 * sizeof(float)));
            CUDACHECK(cudaMemset(d_filtered_hist, 0, proc_cnt * 4 * sizeof(float)));
            CUDACHECK(cudaMemset(d_filtered, 0, proc_cnt * nseg * sizeof(float)));

            mycustom_filter(d_sacdata, d_filtered, d_filtered_tmp, d_a, d_b, d_sac_hist, d_filtered_hist, nseg, proc_cnt);

            CUDACHECK(cudaMemcpy2D(d_sacdata, nseg * sizeof(float),
                                   d_filtered, nseg * sizeof(float),
                                   nseg * sizeof(float), proc_cnt, cudaMemcpyDeviceToDevice));

            CUDACHECK(cudaMemset(d_filtered, 0, proc_cnt * nseg * sizeof(float)));

            // get the filter coefficient from filters
            if (wh_before)
            {
                ...
                freqWhiten(d_spectrum, d_weight, d_tmp_weight, d_tmp, num_ch, nseg, proc_batch, delta, f_idx1, f_idx2, f_idx3, f_idx4);
            ...
            if (do_runabs_mf)
            {
                runabs_mf(d_sacdata, d_sac_sum,
                          d_filtered, d_filtered_tmp,
                          d_a, d_b, d_sac_hist, d_filtered_hist,
                          d_tmp, d_weight, d_tmp_weight, filters,
                          filter_count, delta, proc_batch, num_ch, nseg, MAXVAL);
                // copy d_sac_sum to d_sacdata
                CUDACHECK(cudaMemcpy2D(d_sacdata, nseg * sizeof(float),
                                       d_sac_sum, nseg * sizeof(float),
                                       nseg * sizeof(float), proc_cnt, cudaMemcpyDeviceToDevice));
            }
            ...
            if (do_runabs)
            {
                ...
                runabs(d_sacdata, d_tmp, d_weight, d_tmp_weight, freq_lows_limit, delta, proc_batch, num_ch, nseg, MAXVAL);
            }
            ...
            // Zero-Padding
            preprocess(d_sacdata, d_sum, d_isum, nseg, proc_cnt, TAPER_RATIO); // Post process using the preprocess function
    ...
    GpuFree((void **)&d_sac_sum);
    GpuFree((void **)&d_filtered);
    GpuFree((void **)&d_filtered_hist);
    GpuFree((void **)&d_filtered_tmp);
