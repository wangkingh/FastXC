                       ...
                       cufftHandle *planfwd_output)
{
    ...
    CUDACHECK(cudaMalloc((void **)d_sacdata_2x, num_ch * batch * work_nfft * sizeof(float)));
    CUDACHECK(cudaMalloc((void **)d_spectrum_2x, num_ch * batch * work_nfft * sizeof(cuComplex)));

    if (do_runabs_mf)
    {
        CUDACHECK(cudaMalloc((void **)d_base_spectrum_2x,
